	...
	public static void findWord(File sourceFile,int fileNumber,Matcher matcher,String p1)
	{
		try
    	{
			...
			while ((line = reader.readLine()) != null){
				matcher.reset(line);
				while (matcher.find()) {
					RentalAnalysis.key.add(matcher.group());
				}
			}
			reader.close();
			for(int p = 0; p<RentalAnalysis.key.size(); p++){
				RentalAnalysis.numbers.put(RentalAnalysis.key.get(p), findEditDistance(p1.toLowerCase(),RentalAnalysis.key.get(p).toLowerCase()));
			}
    	}     
    	catch(Exception e)
    	{
